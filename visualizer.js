document.addEventListener("DOMContentLoaded", () => {
  ...
  function drawPolygon(previewX, previewY) {
    ...
    overlayCtx.strokeStyle = "#ff7043";
    overlayCtx.lineWidth = 3;
    overlayCtx.fillStyle = "rgba(255, 112, 67, 0.2)";
  ...
  function renderPresetColors() {
    ...
    Object.values(colors).forEach((rgbColor) => {
      const hexColor = rgbToHex(rgbColor);
      const preset = document.createElement("div");
      preset.className = "preset-color";
      preset.style.background = hexColor;
      preset.dataset.color = hexColor;
      preset.addEventListener("click", () => {
        colorPicker.value = hexColor;
        colorValue.value = hexColor;
        updateSelectedPreset(hexColor);
      });
      presetColorsContainer.appendChild(preset);
    });
  }
